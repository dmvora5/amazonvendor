...
const page = () => {
  // Sample rows data (10 rows for demo)
  const rows = Array(50).fill(null).map((_, index) => ({
    id: index + 1,
    ...Object.fromEntries(Array.from({ length: 10 }, (_, i) => [`Column ${i + 1}`, `Data ${index + 1}-${i + 1}`])),
  }));
  ...
  return (
    <div className="flex-1 p-6 overflow-hidden">
      <div className="relative overflow-x-auto overflow-y-hidden shadow-2xl sm:rounded-lg h-full">
          ...
          <thead className="text-xs uppercase bg-gradient-to-r from-blue-500 to-indigo-500 text-white">
            <tr>
              {Array.from({ length: 10 }, (_, i) => (
                <th key={i} className="px-6 py-4 text-left font-semibold min-w-52">{`Column ${i + 1}`}</th>
              ))}
